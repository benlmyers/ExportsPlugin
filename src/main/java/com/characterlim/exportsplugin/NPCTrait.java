...
public class NPCTrait extends Trait {
    ...
    public void click(NPCLeftClickEvent event) {
        if(event.getNPC() == this.getNPC()) {
            Comm.send(event.getClicker(), "Accepted item: &964x " + getFormattedName(RotationManager.getAcceptedItem()) + "&b | Sell Price: " + EconomyManager.formatted(PriceManager.getSellPrice()) + "&8 (Next Up: &7" + getFormattedName(RotationManager.getNextItem()) + "&8)");
        }
    }
